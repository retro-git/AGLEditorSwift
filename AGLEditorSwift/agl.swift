...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_agl_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_agl_rustbuffer_free(self, $0) }
    }
}
...
public func `compileFfi`(`source`: String, `mode`: Mode)  -> String {
    return try!  FfiConverterString.lift(
        try! rustCall() {
    uniffi_agl_fn_func_compile_ffi(
        FfiConverterString.lower(`source`),
        FfiConverterTypeMode.lower(`mode`),$0)
}
    )
}
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_agl_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_agl_checksum_func_compile_ffi() != 13354) {
